...
// Cuando no hay ning�n otro evento se invoca   a glutIdleFunc 
...
void Idle_cb() {
  ...
  // esperar 1/60 segundos antes de pasar al pr�ximo cuadro
  if (lapso<=16) { return; }
...
void Motion_cb(int xm, int ym){ // drag
  if (modifiers==GLUT_ACTIVE_SHIFT) { // cambio de escala
    ...
    Normalize(eye); Normalize(up); // mata el error num�rico
    xclick=xm; yclick=ym;
    glutPostRedisplay();
  }
}
